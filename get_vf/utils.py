...
def is_valid_gtdb_release(val, parser, var):
    releases = list(gtdb_releases.keys())
    value = str(val)
    if value in releases:
        return value
    else:
        parser.error(
            f"argument {var}: Invalid value {value}. Filter has to be one of {convert_list_to_str(releases)}"
        )
...
def is_in_markers(parser, arg, var):
    if arg in markers:
        return arg
    else:
        parser.error(
            "argument %s: Invalid value %s.\nMarker has to be one of %s"
            % (var, arg, convert_list_to_str(markers))
        )


def is_in_aln_formats(parser, arg, var):
    if arg in aln_formats:
        return arg
    else:
        parser.error(
            "argument %s: Invalid value %s.\nFormat has to be one of %s"
            % (var, arg, convert_list_to_str(aln_formats))
        )
...
help_msg = {
    ...
    "local_db": "Path to a local findMarker DB",
    "marker": "One of the 120 GTDB bacterial marker genes (e.g. TIGR00593)",
    ...
    "db_dir": "Folder with the marker data generated by the fetch subcommand",
...
def get_arguments(argv=None):
    ...
    parser_fetch = sub_parsers.add_parser(
        "fetch",
        help="Fetch GTDB marker data",
        parents=[parent_parser],
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser_search = sub_parsers.add_parser(
        "search",
        help="Search short reads against GTDB marker data",
        parents=[parent_parser],
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ...
    fetch_required_args = parser_fetch.add_argument_group("required arguments")
    fetch_args = parser_fetch.add_argument_group("GTDB fetch arguments")
    prepare_args = parser_fetch.add_argument_group(
    ...
    extract_args = parser_search.add_argument_group("Extract marker reads arguments")
    fetch_required_args.add_argument(
        "--marker",
        required=True,
        metavar="STR",
        default=argparse.SUPPRESS,
        type=lambda x: is_in_markers(parser, x, "--marker"),
        dest="marker",
        help=help_msg["marker"],
    )
    ...
    fetch_args.add_argument(
        ...
        metavar="STR",
        default=argparse.SUPPRESS,
        type=lambda x: is_valid_dir(parser, x, "--local-db"),
        dest="local_db",
        help=help_msg["local_db"],
    )
    ...
    prepare_args.add_argument(
        "--hmmalign-bin",
        default=defaults["hmmalign_bin"],
        metavar="STR",
        type=lambda x: is_executable(parser, x, "--hmmalign-bin"),
        dest="hmmalign_bin",
        help=help_msg["hmmalign_bin"],
    )
    prepare_args.add_argument(
        "--z-score",
        type=lambda x: float(
    ...
    prepare_args.add_argument(
        ...
        required=False,
        metavar="STR",
        default=defaults["aln_format"],
        type=lambda x: is_in_aln_formats(parser, x, "--aln-format"),
        dest="aln_format",
        help=help_msg["aln_format"],
    )
...
def create_output_files(prefix, input, marker, fastq):
    ...
    out_files = {
        ...
        "results": f"{prefix}.results.{marker}.tsv",
        "results_gz": f"{prefix}.results.{marker}.tsv.gz",
        "results_log": f"{prefix}.results.{marker}.log",
        "results_filtered_prefix": f"{prefix}.results-filtered.{marker}",
        "results_filtered_log": f"{prefix}.results-filtered.{marker}.log",
        "results_filtered_mm": f"{prefix}.results-filtered.{marker}_multimap.tsv.gz",
        "results_filtered_cov": f"{prefix}.results-filtered.{marker}_cov-stats.tsv.gz",
        "reads_marker_log": f"{prefix}.reads-marker.{marker}.log",
        "reads_marker": f"{prefix}.reads-marker.{marker}.{extension}.gz",
    }
    return out_files
